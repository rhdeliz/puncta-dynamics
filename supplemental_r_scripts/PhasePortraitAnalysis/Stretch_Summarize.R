...
StatTable <-
  ...
  mutate(
    LIGAND_DENSITY_CAT = ifelse(IMAGE=="20190606 44nm il1 EL4 IRAK1 mScar MyD88 GFP 001", 32, LIGAND_DENSITY_CAT)
  ) %>% 
  group_by(
    IMAGE
  ) %>% 
  ...
  filter(
    ROUNDED_REFERENCE_TOTAL_INTENSITY >= 0,
    ROUNDED_REFERENCE_TOTAL_INTENSITY <= 10,
    ROUNDED_QUERY_TOTAL_INTENSITY >= 0,
