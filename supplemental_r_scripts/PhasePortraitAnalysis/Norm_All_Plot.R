...
# FRAMES_SINCE_LANDING_CAT_BIN <- 150 #Fig2 All
# FRAMES_SINCE_LANDING_CAT_BIN <- 300 #Fig3 T6 N
N_THRESHOLD = 100

setwd(OUTPUT_DIRECTORY)

# if(exists("PhasePortraitAll") == FALSE){
  SaveName <-
    paste0(
...
TempPhasePortraitAll <-
  ...
  ungroup() %>% 
  mutate(
    COLOR = sqrt(DELTA_QUERY_TOTAL_INTENSITY^2 + DELTA_REFERENCE_TOTAL_INTENSITY^2)*60
  ) %>% 
  mutate(
    MIN_COLOR = min(COLOR),
    MAX_COLOR = max(COLOR),
    MAX_X = max(ROUNDED_REFERENCE_TOTAL_INTENSITY),
    MAX_Y = max(ROUNDED_QUERY_TOTAL_INTENSITY)
  ) %>% 
  as.data.table()
    ...
    ggplot(
      ...
      geom_arrow(
        ...
        aes(
          ...
          mag = .5,
          angle = atan2(d_qry_int, d_ref_int)*180/pi,
          color = sqrt(d_qry_int^2 + d_ref_int^2)*60,
      ...
      scale_x_continuous(
        limits = c(-STEP_SIZE/2, max(TempPhasePortraitAll$MAX_X)+STEP_SIZE/2),
        breaks = c(0:10)*round(max(TempPhasePortraitAll$MAX_X)/2)
        # labels = add_x
      ) +
      scale_y_continuous(
        limits = c(-STEP_SIZE/2, max(TempPhasePortraitAll$MAX_Y)+STEP_SIZE/2),
        breaks = c(0:10)*round(max(TempPhasePortraitAll$MAX_X)/2)
        # labels = add_x
      ) +
      ...
      scale_color_viridis_c(
        limits = c(TempPhasePortraitAll$MIN_COLOR[1], TempPhasePortraitAll$MAX_COLOR[1]),
        option = "plasma",
        guide = "none"
        # labels = mult_format
        # direction = -1,
      ...
      labs(
        # title = PlotTitle,
        x = paste(TempTable$REFERENCE_PROTEIN[1], "(mol.)"),
        y = paste(TempTable$QUERY_PROTEIN[1], "(mol.)"),
...
Paths <- mclapply(1:NROW(PlotList), PlotFx)
