...
PlotFx <- function(FacetX){
  tryCatch({
    ...
    geom_arrow(
      ...
      scale_x_continuous(
        limits = c(-1, STEP_SIZE*10),
        
        # limits = c(-STEP_SIZE/2, max(TempPhasePortraitAll$MAX_X)+STEP_SIZE/2),
        breaks = c(0:4)*max(TempPhasePortraitAll$MAX_X)/2
      ) +
      scale_y_continuous(
        limits = c(-1, STEP_SIZE*2)
      ...
      labs(
        # title = PlotTitle,
        x = paste("(x)", TempTable$REFERENCE_PROTEIN[1]),
        y = paste("(x)", TempTable$QUERY_PROTEIN[1]),
        color = "Rate\n(mol./min)",
